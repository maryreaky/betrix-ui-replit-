...
test('UI/UX - Analysis response has structure', async () => {
  const result = await handleCommand('analyze', [], 123, 456, mockRedis, {});
  // When no fixtureId is provided, it asks user to provide one
  assert(result, 'No analyze response');
  assert(result.text, 'Missing text content');
  assert(result.reply_markup?.inline_keyboard, 'Missing action buttons when no ID provided');
