...
test('Betting sites - Kenya sites available with all info', () => {
  const sites = getBettingSitesForCountry('KE');
...
test('Betting sites formatting - creates clickable keyboard', () => {
  const sites = getBettingSitesForCountry('KE');
  const formatted = formatBettingSites(sites);
  
  assert(formatted.text, 'Missing formatted text');
  assert(formatted.reply_markup?.inline_keyboard, 'Missing keyboard');
  assert(formatted.reply_markup.inline_keyboard.length > 0, 'No buttons');
  
  console.log(`✅ Betting sites formatted with ${formatted.reply_markup.inline_keyboard.length} button rows`);
});
