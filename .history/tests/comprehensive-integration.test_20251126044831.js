...
test('UI/UX - Odds response formatting', async () => {
  ...
  assert(result.text.includes('Odds') || result.text.includes('Match') || result.text.includes('vs'), 'Missing odds/match content');
  assert(result.reply_markup?.inline_keyboard, 'Missing filter buttons');
  console.log('✅ Odds formatting includes matches and filter buttons');
});
...
test('UI/UX - Analysis response with confidence and narrative', async () => {
  ...
  assert(result.text.includes('analysis') || result.text.includes('Analysis') || result.text.includes('provide a match'), 'Analysis content not found');
  assert(result.reply_markup?.inline_keyboard, 'Missing action buttons');
  console.log('✅ Analysis response includes content and action buttons');
});
