...
async function handleNews(chatId, userId, redis, services, query = {}) {
  try {
    const { sportsAggregator, rss } = services;

    let articles = [];
    
    // Try live news provider first (Google News, no registration needed)
    if (sportsAggregator && sportsAggregator.getLiveNews) {
      try {
        articles = await sportsAggregator.getLiveNews('football', 8);
        logger.info(`News: fetched ${articles.length} articles from live provider`);
      } catch (e) {
        logger.warn('Failed to fetch live news', e.message);
      }
    }
    
    // Fallback to RSS if live news failed
    if (!articles || articles.length === 0) {
      if (rss) {
        try {
          ...
          logger.warn('Failed to fetch RSS news', e);
        }
      }
    }
