...
export class StatPalInit {
  ...
  async testSportEndpoint(sport, endpoint = 'livescores') {
    try {
      ...
      logger.debug(`🔍 ${sport.toUpperCase()} ${endpoint} RAW RESPONSE TYPE: ${typeof data}, is array: ${Array.isArray(data)}`);
      if (typeof data === 'object' && !Array.isArray(data)) {
        logger.debug(`    Response keys: ${Object.keys(data).slice(0, 10).join(', ')}`);
      }

      logger.info(`✅ ${sport.toUpperCase()} ${endpoint}: ${Array.isArray(data) ? data.length : (typeof data === 'object' ? 'object' : 'string')} items`);
