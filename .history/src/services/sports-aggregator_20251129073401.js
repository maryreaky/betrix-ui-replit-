...
export class SportsAggregator {
   ...
   * Tries cached live data, StatPal live fetch, and SportMonks livescores as fallback.
   */
  async getMatchById(matchId, sport = 'soccer') {
    try {
      ...
      // 1) search small in-memory caches (live caches)
      for (const [k, v] of this.cache.entries()) {
        if (k.startsWith('live:') && v && Array.isArray(v.data)) {
          const found = findIn(v.data);
          if (found) return this._formatMatches([found], 'statpal')[0];
        }
      }

      // 2) ask StatPal live endpoint
      try {
        const live = await this._getLiveFromStatPal(sport, CONFIG.STATPAL.V1 || 'v1');
      ...
      // 3) try SportMonks livescores (if configured)
