  ...
  async _fetch(endpoint, query = {}) {
    try {
      ...
      if (!res.ok) {
        ...
        // Log helpful info without exposing the API token
        const safeUrl = url.replace(/(api_token|api_token=[^&]+)/gi, 'api_token=REDACTED');
        throw new Error(`HTTP ${res.status} ${res.statusText} - ${text.substring(0, 300)} (url: ${safeUrl})`);
      }
      const json = await res.json().catch(() => null);
      return json && (json.data || json) ? (json.data || json) : json;
    } catch (e) {
      logger.warn('SportMonks fetch failed for', endpoint, e?.message || String(e));
      return null;
    }
  }
