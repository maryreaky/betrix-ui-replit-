...
export class SportsAggregator {
  ...
  async _getLiveFromSportsMonks() {
    // Prefer the dedicated livescores endpoint — more reliable for current live matches
    if (!CONFIG.SPORTSMONKS.KEY) return [];
    try {
      // Use /football/livescores which returns currently active live fixtures
      const url = `${CONFIG.SPORTSMONKS.BASE}/football/livescores?api_token=${CONFIG.SPORTSMONKS.KEY}`;
      const response = await this._fetchWithRetry(url);
