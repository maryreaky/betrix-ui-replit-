...
class StatPalService {
  ...
  async getLiveScores(sport = 'soccer', version = 'v1') {
    const healthKey = `statpal:${sport}:livescores`;
    try {
      if (await this.providerHealth.isDisabled(healthKey)) {
        logger.info(`StatPal ${sport} livescores provider currently disabled`);
        return null;
      }
      ...
      try { await this.providerHealth.clear(healthKey); } catch (e) {}
      return data;
    } catch (error) {
      logger.error(`❌ StatPal live scores error (${sport}):`, error.message);
      // Only disable for 404 if it's sport-specific, not if it's an API-level issue
      // Sport-specific 404s are expected for some sports that StatPal doesn't support
      const statusCode = error.statusCode || error.status || (error.message?.includes('404') ? 404 : 500);
      if (statusCode === 404) {
        logger.info(`⚠️  ${sport} not available on StatPal (404), will try other providers`);
