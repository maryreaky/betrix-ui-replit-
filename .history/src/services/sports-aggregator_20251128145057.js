...
export class SportsAggregator {
  ...
  async getOdds(leagueId, options = {}) {
    try {
      ...
      if (!CONFIG.STATPAL.KEY) {
        logger.error('❌ StatPal API Key not configured for odds');
        return [];
      }

      try {
        logger.debug(`📊 Fetching odds from StatPal`);
        const odds = await this.statpal.getLiveOdds('soccer', 'v1');
        
        if (odds && (Array.isArray(odds) ? odds.length > 0 : Object.keys(odds).length > 0)) {
          const oddsList = Array.isArray(odds) ? odds : (odds.data || odds.odds || []);
          if (oddsList.length > 0) {
            logger.info(`✅ StatPal: Found ${oddsList.length} odds`);
            this._setCached(cacheKey, oddsList);
            await this._recordProviderHealth('statpal-odds', true, `Found ${oddsList.length} odds`);
            return oddsList;
          }
        }

        logger.warn('⚠️  StatPal returned no odds data');
        await this._recordProviderHealth('statpal-odds', false, 'No odds available');
        return [];
      } catch (e) {
        logger.error(`❌ StatPal odds error: ${e.message}`);
        await this._recordProviderHealth('statpal-odds', false, e.message);
        return [];
      }
    } catch (err) {
      logger.error('getOdds failed:', err.message);
      return [];
    }
  }

  /**
   * Get upcoming fixtures - Using StatPal
   */
  async getUpcomingFixtures(leagueId, options = {}) {
    try {
      ...
      if (this.cache.has(cacheKey)) {
        const cached = this.cache.get(cacheKey);
        if (Date.now() - cached.timestamp < 60 * 60 * 1000) { // 1 hour cache for fixtures
          return cached.data;
        }
      }

      if (!CONFIG.STATPAL.KEY) {
        logger.error('❌ StatPal API Key not configured for fixtures');
        return [];
      }

      try {
        logger.debug(`📅 Fetching fixtures from StatPal`);
        const fixtures = await this.statpal.getFixtures('soccer', 'v1');
        
        if (fixtures && (Array.isArray(fixtures) ? fixtures.length > 0 : Object.keys(fixtures).length > 0)) {
          const fixturesList = Array.isArray(fixtures) ? fixtures : (fixtures.data || fixtures.fixtures || []);
          if (fixturesList.length > 0) {
            logger.info(`✅ StatPal: Found ${fixturesList.length} upcoming fixtures`);
            this._setCached(cacheKey, fixturesList);
            await this._recordProviderHealth('statpal-fixtures', true, `Found ${fixturesList.length} fixtures`);
            return fixturesList;
          }
        }

        logger.warn('⚠️  StatPal returned no fixtures data');
        await this._recordProviderHealth('statpal-fixtures', false, 'No fixtures available');
        return [];
      } catch (e) {
        logger.error(`❌ StatPal fixtures error: ${e.message}`);
        await this._recordProviderHealth('statpal-fixtures', false, e.message);
        return [];
      }
    } catch (err) {
      logger.error('getUpcomingFixtures failed:', err.message);
      return [];
    }
  }

  /**
   * Get standings - Using StatPal
   */
  async getStandings(leagueId, options = {}) {
    try {
      ...
      if (!CONFIG.STATPAL.KEY) {
        logger.error('❌ StatPal API Key not configured for standings');
        return [];
      }

      try {
        logger.debug(`🏆 Fetching standings from StatPal`);
        const standings = await this.statpal.getStandings('soccer', null, 'v1');
        
        if (standings && (Array.isArray(standings) ? standings.length > 0 : Object.keys(standings).length > 0)) {
          const standingsList = Array.isArray(standings) ? standings : (standings.data || standings.standings || []);
          if (standingsList.length > 0) {
            logger.info(`✅ StatPal: Found standings data`);
            this._setCached(cacheKey, standingsList);
            await this._recordProviderHealth('statpal-standings', true, 'Standings fetched');
            return standingsList;
          }
        }

        logger.warn('⚠️  StatPal returned no standings data');
        return [];
      } catch (e) {
        logger.error(`❌ StatPal standings error: ${e.message}`);
        return [];
      }
    } catch (err) {
      logger.error('getStandings failed:', err.message);
      return [];
    }
  }

  /**
   * Utility: Record provider health status
   */
